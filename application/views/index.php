<?php
$this->db->select('STATUS, COUNT(ID_TICKET) AS TICKET_COUNT');
...
foreach ($result as $ticket) {
    ...
    if ($ticket['STATUS'] === 'finished') {
        $finishedTickets = $ticket['TICKET_COUNT'];
    } elseif ($ticket['STATUS'] === 'unfinished' || $ticket['STATUS'] === 'on-progress') {
        $unfinishedTickets += $ticket['TICKET_COUNT'];
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <link rel="icon" type="image/png" sizes="16x16" href="<?= base_url('assets/'); ?>images/ubs1.1.png">
...
<body>
    ...
    <div class="content">
        <div class="">
            ...
            <!-- Bagian Statistik Email -->
            <div class="px-3 mb-6">
                <div class="row justify-content-between">
                    <div
                        class="col-6 col-md-4 col-xxl-2 text-center border-start-xxl border-end-xxl-0 border-end pb-4 pb-xxl-0">
                        <span class="uil fs-3 lh-1 uil-envelope text-primary"></span>
                        <h1 class="fs-3 pt-3"><?= number_format($totalTickets) ?></h1>
                        <p class="fs--1 mb-0">Total Tickets</p>
                    </div>
                    <div
                        class="col-6 col-md-4 col-xxl-2 text-center border-start-xxl border-end-xxl-0 border-end-md pb-4 pb-xxl-0">
                        <span class="uil fs-3 lh-1 uil-envelope-upload text-info"></span>
                        <h1 class="fs-3 pt-3"><?= number_format($finishedTickets) ?></h1>
                        <p class="fs--1 mb-0">Finished Tickets</p>
                    </div>
                    <div
                        class="col-6 col-md-4 col-xxl-2 text-center border-start-xxl border-end border-end-md-0 pb-4 pb-xxl-0 pt-4 pt-md-0">
                        <span class="uil fs-3 lh-1 uil-envelopes text-primary"></span>
                        <h1 class="fs-3 pt-3"><?= number_format($unfinishedTickets) ?></h1>
                        <p class="fs--1 mb-0">Unfinished Tickets</p>
                    </div>
                </div>
            </div>
            <div class="row g-3 mb-5">
                <div class="col-md-6 col-xxl-3">
                    <div class="card h-100">
                        <div class="card-body">
                            <div class="pb-4 pt-3">
                                <div id="status-chart" style="min-height: 300px; width:100%;"></div>
                            </div>
                            <div>
                            </div>
                        </div>
                    ...
                    <div class="card h-100">
                        <div class="card-body d-flex flex-column">
                            <div class="d-flex justify-content-center pt-3 flex-1">
                                <div id="category-chart" style="min-height: 320px; width:100%;"></div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            // Request data for the status chart
            $.ajax({
                url: '<?= base_url("index.php/UserController/get_ticket_status") ?>',
                method: 'GET',
                dataType: 'json',
                success: function (response) {
                    ...
                    // Load the status chart
                    loadStatusChart(statusData);
                },
            });

            // Request data for the category chart
            $.ajax({
                url: '<?= base_url("index.php/UserController/get_ticket_category") ?>',
                method: 'GET',
                dataType: 'json',
                success: function (response) {
                    let categoryData = {};
                    ...
                    response.forEach(ticket => {
                        categoryData[ticket.CATEGORY] = (categoryData[ticket.CATEGORY] || 0) + 1;
                    });
        ...
        // Render status chart with ECharts
        function loadStatusChart(data) {
            console.log("Loading status chart with data:", data);

            const chartDom = document.getElementById('status-chart');
            const statusChart = echarts.init(chartDom);
        ...
        // Render category chart with ECharts
        function loadCategoryChart(data) {
            console.log("Loading category chart with data:", data);
        ...
        // Generate random color if no specific color is available
